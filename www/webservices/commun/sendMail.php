...
function sendMultiPartMail ($to, $cc, $bcc, $from, $subject, $htlmMsg=null, $replyTo=null, $joinFilesName=null, $joinFilesContent=null) {
	...
	if ($htlmMsg != null) {
		...
		preg_match_all('~background.*?url.*?\(([^\)]+)\)~si', $str, $matches);
		foreach ($matches[1] as $img) {
			// Supprimer les caracteres \", \', " ou ' que l'on a pas reussi de filtre dans la recherche si on utilise des \ pour escaper les quotes ou double-quotes
